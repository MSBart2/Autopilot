# Documentation

This directory contains operational documentation for the pipeline-first Cyberpilot repository.

## Core Docs

- [../README.md](../README.md) - repository overview and quick start
- [../architecture.md](../architecture.md) - current technical architecture
- [../AI-SDLC.md](../AI-SDLC.md) - local, cloud, and SDK AI-SDLC pipeline modes

## Operational Docs

- [configuration.md](configuration.md) - current configuration surface
- [testing.md](testing.md) - test projects and commands
- [health-endpoint.md](health-endpoint.md) - health routes
- [ci-cd.md](ci-cd.md) - CI/CD guidance
- [build-performance.md](build-performance.md) - build optimization notes
- [conventions.md](conventions.md) - development conventions

Removed web-demo documentation was intentionally deleted with the demo features it described.
