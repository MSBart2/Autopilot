...
| a06a97057aed451aa6e43c1db0bf8bdd | m4-review-file-validation-tools (#34/#51) | review | claude-sonnet-4.6 | b9c7536 | 55aa422 | 306,581 | 4,799 | 260,035 | 0 | 8 | 13 | 5 | 109,114 | $0.9917 | ✅ | ✅ | Added file-content and validation tools; validation narrative improved but both new tools failed due result shape/size. |
| 1c2a7010171846bb83eb1a9fe2c7da99 | m4-review-file-validation-tools-compact (#34/#51) | review | claude-sonnet-4.6 | 3388907 | 55aa422 | 255,513 | 4,642 | 219,079 | 0 | 7 | 12 | 3 | 104,585 | $0.8362 | ✅ | ✅ | Validation tool stabilized: 0 validation failures. File-content tool still failed 3x. Best token/cost run. |
| c74ce041332c4d3aa95bbd0ec8aef1e7 | m4-review-file-validation-tools-tiny (#34/#51) | review | claude-sonnet-4.6 | e7ae86f | 55aa422 | 340,391 | 5,524 | 301,871 | 0 | 9 | 17 | 5 | 116,120 | $1.1040 | ✅ | ✅ | Smaller file-content output reduced file tool failures to 1 but worsened tokens vs compact. |
| 631d3a1bf6e542b9ab2f6206a52ca55e | m5-review-output-shaping (#34/#51) | review | claude-sonnet-4.6 | e31fc02 | 55aa422 | 225,161 | 5,183 | 198,602 | 0 | 12 | 16 | 12 | 104,943 | $0.7532 | ✅ | ✅ | First M5 shaping attempt lowered tokens but replaced SDK tool success envelopes with plain strings, causing false `tool_execution_failed` metrics. Do not use as final result. |
| b880dcefd12446ccb30ffd2dfcb5719c | m5-review-output-shaping-envelope (#34/#51) | review | claude-sonnet-4.6 | d71a355 | 55aa422 | 178,700 | 4,728 | 154,795 | 0 | 10 | 16 | 0 | 97,215 | $0.6070 | ✅ | ✅ | Corrected M5 output shaping preserves SDK `resultType=success` envelopes while compacting model-facing output and persisting detailed redacted artifacts. Best review result so far. |
| 89e5221bfed94039bef1f4fe5263f855 | m6-triage-family-tiered-costfix (#34) | triage | claude-haiku-4.5 | f0f4df3 | 55aa422 | 150,618 | 4,935 | 136,517 | 0 | 9 | 10 | 0 | 57,562 | $0.1402 | ✅ | ✅ | Global model remained `claude-sonnet-4.6`; family-tiered resolver selected `claude-haiku-4.5`. Earlier M6 run proved selection but cost was misestimated with the global model; fixed in `f0f4df3`. |
...
| Milestone 4 validation + changed-file tools | #34/#51 review-only, same PR head | `m4-review-comment-renderer-compact-20260519` | `m4-review-file-validation-tools-compact-20260519` | -158,863 (-38%) | -4 | -20,190 (-16%) | 3/3 GO; validation failures 1→0; file-read failures 5 absolute view → 3 file-tool failures | Keep `collect_validation_evidence`; `get_changed_file_content` is promising but needs deeper SDK tool-result hardening. Compact run is best overall M4 review run so far. |
| Milestone 5 output shaping | #34/#51 review-only, same PR head | `m4-review-file-validation-tools-compact-20260519` | `m5-review-output-shaping-envelope-20260519` | -76,813 (-30%) | +3 | -7,370 (-7%) | 2/2 GO; corrected run cut failed calls 3→0 and preserved 19 artifact rows / 41,174 chars of redacted detail | Keep soft post-tool shaping. Important SDK lesson: modified results must preserve the SDK success envelope (`resultType=success`) or successful tools are counted as failures. |
| Milestone 6 family-tiered model selection | #34 triage-only, global model stayed Sonnet | `m3-full-20260518` triage | `m6-triage-family-tiered-costfix-20260519` | -12,321 (-8%) | -1 | -20,881 (-27%) | 2/2 GO; selected model changed `claude-sonnet-4.6`→`claude-haiku-4.5`; failed calls 3→0; cost -$0.4031 (-74%) | Keep family-tiered defaults for cheap stages. Fix included: cost estimation must use selected stage model, not run-level model. |
...
### Milestone 4 validation and changed-file tools (`m4-review-file-validation-tools-*`)

Fixture: same issue #34 PR #51 head commit `55aa4226a05803984a5f203525a86a2078e94c57`. This promoted `collect_validation_evidence` for deterministic `dotnet build` / `dotnet test` runs and `get_changed_file_content` for repository-relative file reads with line numbers.

| Run | Cyberpilot SHA | Input tokens | Output tokens | Cache read | Turns | Tool calls | Failed calls | Duration ms | Cost | Failure profile | Result |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- |
| Renderer compact baseline (`m4-review-comment-renderer-compact-20260519`) | `a1d422b` | 414,376 | 5,980 | 377,240 | 11 | 19 | 6 | 124,775 | $1.3328 | 5 absolute-path view failures, 1 PowerShell failure | GO / approved |
| File + validation first attempt (`m4-review-file-validation-tools-20260519`) | `b9c7536` | 306,581 | 4,799 | 260,035 | 8 | 13 | 5 | 109,114 | $0.9917 | 3 file-content failures, 2 validation failures | GO / approved |
| Compact output (`m4-review-file-validation-tools-compact-20260519`) | `3388907` | 255,513 | 4,642 | 219,079 | 7 | 12 | 3 | 104,585 | $0.8362 | 3 file-content failures, 0 validation failures | GO / approved |
| Tiny file output (`m4-review-file-validation-tools-tiny-20260519`) | `e7ae86f` | 340,391 | 5,524 | 301,871 | 9 | 17 | 5 | 116,120 | $1.1040 | 4 absolute-path view failures, 1 file-content failure | GO / approved |

Takeaway: `collect_validation_evidence` is a clear win and eliminated ad hoc PowerShell validation failure. `get_changed_file_content` changed the failure mode but still needs follow-up hardening: compact output was the best overall run, while tiny output reduced file-tool failures but caused the model to fall back to absolute-path `view` calls. Keep the validation tool; treat file-content as partially promoted and investigate SDK tool-result failure details before relying on it as the primary changed-file reader.

### Milestone 5 output shaping (`m5-review-output-shaping-*`)

Fixture: same issue #34 PR #51 head commit `55aa4226a05803984a5f203525a86a2078e94c57`. This tested post-tool output shaping that redacts secret-looking values, replaces noisy model-facing output with compact summaries/tail excerpts, and preserves larger redacted detail in `tool-output-*-detail` artifacts when `CaptureToolOutputArtifacts` is enabled.

| Run | Cyberpilot SHA | Input tokens | Output tokens | Cache read | Turns | Tool calls | Failed calls | Duration ms | Cost | Artifact rows / chars | Failure profile | Result |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- |
| M4 compact baseline (`m4-review-file-validation-tools-compact-20260519`) | `3388907` | 255,513 | 4,642 | 219,079 | 7 | 12 | 3 | 104,585 | $0.8362 | 1 / 571 | 3 file-content failures | GO / approved |
| First M5 shaping attempt (`m5-review-output-shaping-20260519`) | `e31fc02` | 225,161 | 5,183 | 198,602 | 12 | 16 | 12 | 104,943 | $0.7532 | 18 / 48,371 | False failures from replacing SDK tool success envelopes with plain strings | GO / approved |
| Corrected envelope-preserving M5 (`m5-review-output-shaping-envelope-20260519`) | `d71a355` | 178,700 | 4,728 | 154,795 | 10 | 16 | 0 | 97,215 | $0.6070 | 19 / 41,174 | 0 failed tools; detailed redacted artifacts retained | GO / approved |

Takeaway: M5 is a meaningful review-stage win after preserving the SDK result envelope. Compared with the best M4 review run, corrected output shaping cut input tokens by 76,813 (-30%), cache-read tokens by 64,284 (-29%), failed tool calls from 3 to 0, duration by 7,370 ms (-7%), and estimated cost by $0.2291 (-27%). Human auditability improved because diagnostic runs now retain detailed redacted tool artifacts, but the first attempt proved hooks are fragile if they alter SDK result shape.

### Milestone 6 family-tiered model selection (`m6-triage-family-tiered-*`)

Fixture: issue #34 triage-only. The global run model stayed `claude-sonnet-4.6`, but the family-tiered resolver selected `claude-haiku-4.5` for triage. The first M6 run (`m6-triage-family-tiered-20260519`, SHA `1546a6f`) proved stage selection worked but exposed a cost-accounting bug: `EstimatedCostUsd` still used the run-level Sonnet model. SHA `f0f4df3` fixed both persistence sinks to estimate cost from the selected stage model.

| Run | Cyberpilot SHA | Selected model | Input tokens | Output tokens | Cache read | Turns | Tool calls | Failed calls | Duration ms | Cost | Result |
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| M3 triage baseline (`m3-full-20260518`) | `6842057` | `claude-sonnet-4.6` | 162,939 | 3,633 | 143,537 | 10 | 16 | 3 | 78,443 | $0.5433 | GO / approved |
| M6 first run, bad cost accounting (`m6-triage-family-tiered-20260519`) | `1546a6f` | `claude-haiku-4.5` | 257,407 | 5,694 | 233,519 | 14 | 17 | 1 | 72,042 | $0.8576 | GO / approved, cost incorrectly estimated as Sonnet |
| M6 corrected cost run (`m6-triage-family-tiered-costfix-20260519`) | `f0f4df3` | `claude-haiku-4.5` | 150,618 | 4,935 | 136,517 | 9 | 10 | 0 | 57,562 | $0.1402 | GO / approved |

Takeaway: family-tiered selection produced the expected cost win while preserving quality on a cheap stage. Compared with the M3 Sonnet triage baseline, corrected Haiku triage cut cost by $0.4031 (-74%), duration by 20,881 ms (-27%), input tokens by 12,321 (-8%), and failed tool calls from 3 to 0. The implementation keeps `implement` and `review` on the global base model and only tiers `triage`, `plan`, `docs`, and `deliver` within the same model family.
