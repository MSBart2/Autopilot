...
- runs `triage -> plan -> implement -> review -> docs -> summary -> deliver`
...
The default definition is `cyberpilot-default`, which runs `triage -> plan -> implement -> review -> docs -> summary -> deliver`. The SDK also ships these built-ins:

| Definition | Stages | Use case |
|------------|--------|----------|
| `cyberpilot-default` | `triage -> plan -> implement -> review -> docs -> summary -> deliver` | Full issue-to-PR SDLC |
| `bugfix` | `plan -> implement -> review -> summary -> deliver` | Focused implementation fixes when triage/docs are unnecessary |
| `docs-only` | `docs -> summary -> deliver` | Documentation-only changes and landing reports |
