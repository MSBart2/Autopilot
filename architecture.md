...
SDK stage policy allows triage, plan, review, and deliver to run investigative commands, searches, builds, tests, and scripts, but blocks durable side effects such as issue/PR comments, label edits, branch creation, file writes, commits, pushes, PR merges, and direct API mutations. Those stages return intended comments, labels, branch names, plans, verdicts, or landing reports as structured artifacts; write-enabled stages (`implement`, `docs`) or harness-owned code perform durable writes when appropriate. The mutation guard applies across shell and script wrappers so Python/Node subprocess calls cannot bypass the same policy. After a GO deliver result, `PipelineDeliveryCoordinator` loads PR readiness with structured `gh pr view` output, verifies approval/check/mergeability state, and performs `gh pr merge --squash --delete-branch` from harness code before final labels and issue closure.

Review and docs stages can also call deterministic tools for fresh PR metadata, diff summaries, repository-relative file content, validation evidence, and artifact-ready comment bodies instead of searching issue comments, issuing absolute-path reads, inventing shell commands, or attempting blocked GitHub writes. Tool outputs stay compact by default and include artifact references when detailed JSON has been captured for the run ledger.
...
Run details include an operational Run Room with issue title/body context, live SignalR agent output, compact per-stage usage metrics, linked standalone documents for every stage (`Triage`, `Plan`, `Implement`, `Review`, `Docs`, and `Deliver`) rendered from the latest stage `StageResultJson`/evidence, continuation for terminal runs, cancellation for active runs, and Reset Mission for replay testing. The Run Room cards stay compact and link to full-fidelity stage documents for captured output, artifacts, evidence, required actions, and issue/PR links. Reset Mission removes SDK stage labels while preserving the base `sdk` label, deletes recognizable Cyberpilot issue comments, deletes the SDK issue branch locally/remotely when present, and removes the local `PipelineRun` record with its cascaded `PipelineStageLog` rows. Reset Mission is intentionally unavailable once a run completes delivery, because the associated code has already been merged.
