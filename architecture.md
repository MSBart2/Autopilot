...
**Deterministic SDK tools:** `CopilotStageRunner` attaches harness-owned tools from [copilot-sdk/Copilot/PipelineContextToolProvider.cs](copilot-sdk/Copilot/PipelineContextToolProvider.cs) to every SDK session. The current tools are `get_pipeline_context`, `get_pr_details`, `get_pr_diff_summary`, `get_pr_checks`, `render_stage_comment`, `get_changed_file_content`, and `collect_validation_evidence`. They return compact typed results for model consumption and structured errors such as `missing_pr`, `pr_details_failed`, `pr_diff_summary_failed`, `pr_checks_failed`, `unsupported_comment_kind`, `invalid_path`, and `unsupported_validation`. PR tools use `gh pr view --json ...` and `gh pr checks --json ...` through the existing GitHub CLI abstraction. `get_pr_diff_summary` returns changed-file stats, top-directory and extension groups, and deterministic review signals such as `production_code_changed`, `test_code_changed`, `documentation_changed`, `web_surface_changed`, and `configuration_changed` so review/docs stages can avoid rediscovering the PR shape with shell commands. `get_pr_checks` returns structured status/check rows including CodeQL/build/test names, state, bucket, workflow, and links so agents do not need ad hoc PowerShell status parsing. `get_changed_file_content` reads repository-relative paths with line numbers so stages avoid absolute-path `view` failures. `collect_validation_evidence` runs known `dotnet build` / `dotnet test` commands without shell syntax and returns compact exit/duration/output-tail evidence. `render_stage_comment` returns deterministic Markdown for started/progress/verdict/verification/landing updates without posting to GitHub; stages include that body in the required artifact while policy-controlled harness code owns durable writes. Detailed JSON output can be recorded on `PipelineExecutionContext` as `tool-output-*` artifacts when `CaptureToolOutputArtifacts` is enabled; normal runs leave those verbose diagnostics out of `PipelineArtifacts` so the artifact ledger stays focused on stage-owned outputs.

**Stage tool policy hooks:** SDK sessions also attach `StageToolPolicyHooks`, which use `SessionHooks.OnPreToolUse` and `OnPostToolUse`. Pre-tool policy allows Cyberpilot's deterministic read tools, denies broad write-looking operations in read-only stages, and leaves write-capable stages (`implement`, `docs`) able to perform code and documentation actions. Delivery-side mutations are handled by harness code rather than the Deliver model. Post-tool policy uses a soft output-shaping policy: secret-looking values are redacted, noisy results are replaced with a compact model-facing summary plus tail excerpt, and diagnostic runs can persist larger redacted detail as `tool-output-*-detail` artifacts when `CaptureToolOutputArtifacts` is enabled. The hook narrows context blast radius while preserving human auditability.

**MCP evaluation:** MCP was evaluated but is not currently wired into the SDK harness. GitHub and SQLite/database access remain native Cyberpilot tools because those paths need deterministic payload shapes, explicit error codes, mutation guards, and first-class run-history evidence. Filesystem MCP was the closest candidate, but it does not beat existing local read/search tooling enough to justify the added process, dependency, permission, and troubleshooting surface. Revisit MCP only when a mature server provides clear capability Cyberpilot cannot match with simpler native tools.

**Per-stage model selection:** `StageModelResolver` checks the configured model before each stage starts. Stages have family-tiered defaults when no explicit override is supplied: `triage`, `docs`, and `deliver` use the small tier (`claude-haiku-4.5` / `gpt-5-mini`), while `plan`, `implement`, and `review` use at least the medium tier (`claude-sonnet-4.6` / `gpt-5.4`) and never downgrade a large run-level model. Synthetic review dimensions such as `review:security` inherit the base `review` tier and `--stage-model review=<model>` override unless a dimension-specific override is supplied. Stage results can emit `recommended_model_tier` (`small`, `medium`, or `large`); prior-stage recommendations can only escalate `plan`, `implement`, or `review`, never silently downgrade them. The large tier maps to `claude-opus-4.6` / `gpt-5.5`. If a family-tiered model is unavailable, the resolver falls back to the global base model. Operators can still use `--stage-model <stage>=<model>` to override one stage or `*`, and `--stage-fallback-model <stage>=<model>` to provide an explicit fallback. Web-triggered runs can pass the same maps through request-level stage model override/fallback fields. Selected model, configured model, fallback model, and fallback reason are recorded on each `PipelineStageLog`.

**SDK session persistence:** `CopilotStageRunner` assigns deterministic session IDs using `cyberpilot-{runId}-{stageName}-{attempt}` and passes them through `SessionConfig.SessionId`, creating resumable SDK session state under the Copilot session store. Stage completion records the stable `SdkSessionId`, lifecycle state, resume eligibility, blocked reason, and cleanup deadline on `PipelineStageLog`. Successful idle stages are marked `not_applicable` for resume, interrupted stages with a stable session ID are marked `eligible`, and aborted or session-error cases fail closed with a blocked reason. Live steering/queueing is intentionally deferred until there is a stronger product need.
...
- `SdkSessionId`, `SessionState`, `ResumeEligibility`, `ResumeBlockedReason`, `SessionCleanupAfter` (nullable; added in migration `AddSdkSessionPersistence`) — stable SDK session identity, fail-closed resume status, and explicit cleanup window
...
| `PipelinesController` | `/Pipelines`, `/Pipelines/Issues`, `/Pipelines/{id}`, `/Pipelines/{id}/Triage`, `/Pipelines/{id}/Plan`, `/Pipelines/{id}/Implement`, `/Pipelines/{id}/Review`, `/Pipelines/{id}/Docs`, `/Pipelines/{id}/Summary`, `/Pipelines/{id}/Deliver`, `/Pipelines/{id}/Continue`, `/Pipelines/{id}/RetryStage`, `/Pipelines/{id}/ResetMission`, `/Pipelines/Guide/{mode}` | Pipeline modes, stages, run history, issue launcher, run details, stage output documents, run continuation, stage retry, replay reset, and Markdown guides |
...
Local mode uses [.github/agents/cyberpilot.agent.md](.github/agents/cyberpilot.agent.md) as the controller agent. It delegates to specialist agents for triage, planning, implementation, review, docs, summary, and delivery.
...
SDK stage policy allows triage, plan, review, summary, and deliver to run investigative commands, searches, builds, tests, and scripts, but blocks durable side effects such as issue/PR comments, label edits, branch creation, file writes, commits, pushes, PR merges, and direct API mutations. Those stages return intended comments, labels, branch names, plans, verdicts, summary reports, or landing reports as structured artifacts; write-enabled stages (`implement`, `docs`) or harness-owned code perform durable writes when appropriate. The mutation guard applies across shell and script wrappers so Python/Node subprocess calls cannot bypass the same policy. After a GO deliver result, `PipelineDeliveryCoordinator` loads PR readiness with structured `gh pr view` output, verifies approval/check/mergeability state, and performs `gh pr merge --squash --delete-branch` from harness code before final labels and issue closure.

Review, docs, and summary stages can also call deterministic tools for fresh PR metadata, diff summaries, repository-relative file content, validation evidence, and artifact-ready comment bodies instead of searching issue comments, issuing absolute-path reads, inventing shell commands, or attempting blocked GitHub writes. Tool outputs stay compact by default and include artifact references when detailed JSON has been captured for the run ledger.

`--parallel-review-dimensions` changes only the SDK review stage. Instead of one review session, the harness dispatches read-only `review:architecture`, `review:security`, `review:quality`, `review:tests`, and `review:docs` sessions concurrently with isolated progress capture. The parent run emits `review_dimension` dispatch events for start/completion visibility, persists each dimension as a normal stage-log row, and then merges all dimension outputs through `ReviewDimensionAggregator`. The aggregate verdict is deterministic: invalid/non-GO dimensions produce `STOP`, any dimension requesting changes produces `changes_requested`, and all-clean dimensions produce `approved`. The resulting `review-verdict` artifact includes the participant table, per-dimension status/decision, duration, and token summary.
...
Run details include an operational Run Room with issue title/body context, live SignalR agent output, compact per-stage usage metrics, linked standalone documents for every stage (`Triage`, `Plan`, `Implement`, `Review`, `Docs`, `Summary`, and `Deliver`) rendered from the latest stage `StageResultJson`/evidence, continuation for terminal runs, cancellation for active runs, and Reset Mission for replay testing. The Run Room cards stay compact and link to full-fidelity stage documents for captured output, artifacts, evidence, required actions, and issue/PR links. Reset Mission removes SDK stage labels while preserving the base `sdk` label, deletes recognizable Cyberpilot issue comments, deletes the SDK issue branch locally/remotely when present, and removes the local `PipelineRun` record with its cascaded `PipelineStageLog` rows. Reset Mission is intentionally unavailable once a run completes delivery, because the associated code has already been merged.
